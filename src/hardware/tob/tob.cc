...
namespace kiwi::hardware {
    ...
    auto TOB::bump(std::usize bump_index) -> Bump* {
    ...
    auto TOB::all_bump_to_hori_reg() -> std::Vector<int>
    {
        std::Vector<int> regValue;
        for (auto& tobmux: _bump_to_hori_muxs)
        {
            for(auto& muxvalue: tobmux.whole_reg())
            {
                regValue.push_back(muxvalue);
            }
        }
        assert(regValue.size() == 128);

        return std::move(regValue);
    }

    auto TOB::all_hori_to_vert_reg() -> std::Vector<int>
    {
        std::Vector<int> regValue;
        for (auto& tobmux: _hori_to_vert_muxs)
        {
            for(auto& muxvalue: tobmux.whole_reg())
            {
                regValue.push_back(muxvalue);
    ...
    auto TOB::all_vert_to_track_reg() -> std::Vector<int>
    {
        std::Vector<int> regValue;
        for (auto& tobmux: _vert_to_track_muxs)
        {
            for(auto& muxvalue: tobmux.whole_reg())
            {
                regValue.push_back(muxvalue);
    ...
    //Q: bump to tob 是指从 bump 到 die？
    //0 为 bump 到 die，1是 die 到 bump
    auto TOB::all_bump_dir_reg() -> std::Vector<int>
    {
        std::Vector<int> regValue;
        for (size_t i = 0; i < _bump_dir_registers.size(); ++i)
        {
            switch(_bump_dir_registers[i].get())
            {
                case hardware::TOBBumpDirection::BumpToTOB:
                    regValue[i] = 0;
                    break;
                case hardware::TOBBumpDirection::TOBToBump:
                    regValue[i] = 1;
                    break;
                default:
                    std::cerr << "bump dir disconnected" << std::endl;
                    exit(7);
            }
        }
        assert(regValue.size() == 128);
    ...
    // 0 是 track 到 tob，1 是 tob 到 track
    auto TOB::all_track_dir_reg() -> std::Vector<int>
    {
        std::Vector<int> regValue;
        for (size_t i = 0; i < _track_dir_registers.size(); ++i)
        {
            switch(_track_dir_registers[i].get())
            {
                case hardware::TOBTrackDirection::TOBToTrack:
                    regValue[i] = 0;
                    break;
                case hardware::TOBTrackDirection::TrackToTOB:
                    regValue[i] = 1;
                    break;
                default:
                    std::cerr << "track to tob disconnected" << std::endl;
                    exit(7);
            }
        }
        assert(regValue.size() == 128);
