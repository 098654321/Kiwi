...
namespace kiwi::hardware {
    ...
    //updated by tang, 2024-09-27
    auto COB::all_sel_reg_value(COBDirection dir) -> std::Array<COBSelRegister, 128> {
        std::Array<COBSelRegister, 128> all_sel_reg;
        for (int i = 0; i < 16; ++i)
        {
            std::Array<COBSelRegister, 8> unitreg = _cobunits[i].whole_sel_reg(dir);
            for (int j = 0; j < 8; ++j)
            {
                all_sel_reg[i*8+j] = unitreg[j];
            }
        }
        return std::move(all_sel_reg);
    }

    auto COB::all_sw_reg_value(COBDirection from_dir, COBDirection to_dir) -> std::Array<COBSwState, 128> {
        std::Array<COBSwState, 128> all_sw_reg;
        for (int i = 0; i < 16; ++i)
        {
            std::Array<COBSwRegister, 8> unitreg = _cobunits[i].whole_sw_reg(from_dir, to_dir);
            for (int j = 0; j < 8; ++j)
            {
                all_sw_reg[i*8+j] = unitreg[j].get();
            }
        }
        return std::move(all_sw_reg);
    }
    //
